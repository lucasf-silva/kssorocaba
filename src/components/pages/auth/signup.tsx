...
export const SignUp = () => {
    const signInTranslations = useTranslations('SignUpPage')
    ...
    const handleSignInBtn = async () => {
        ...
        const signup = await signUp({ data: { name: data.name, email: mail } })

        setLoading(false)
        if (signup?.error) {
            setError(errorsTranslations(`auth/${signup.error}`))
        }
    }
                    ...
                    <section className='w-full flex-1 flex flex-col justify-center items-center'>
                        ...
                        <div className='w-full max-w-xl flex justify-center items-center flex-col gap-4 p-2'>
                            ...
                            <p className="mt-7 text-center">
                                {signInTranslations('already_have_account')}
                                <Link
                                    href='/auth/signin'
                                    className="text-blue-500 ml-1"
                                >
                                    {signInTranslations('btn_have_account_label')}
                                </Link>
                            </p>
                        </div>
                    </section>
